...
interface BeritaAcaraVendorPayload {
  vendorName: string;
  vendorPic: string;
  cabang: string;
  tgl: string;
...
function generateBeritaAcaraVendorHtml(payload: BeritaAcaraVendorPayload): string {
  ...
  const cabangText = payload.cabang
    ? (payload.cabang.startsWith("Cabang") ? payload.cabang : `Cabang ${payload.cabang}`)
    : "Bank Nagari";

  const vendorDisplayName = payload.vendorName ? payload.vendorName.toUpperCase() : "VENDOR PERBAIKAN";
...
<head>
<meta charset="utf-8">
<title>BERITA ACARA VENDOR PERBAIKAN - ${payload.noTiket || vendorDisplayName}</title>
<!--[if gte mso 9]>
<xml>
 <w:WordDocument>
  <w:View>Print</w:View>
...
<body>
  ...
  <div class="WordSection1" style="width: 480pt; max-width: 100%; margin: 0 auto;">
    ...
    <div class="title-section" align="center" style="text-align: center; margin: 20px 0 24px 0;">
      ...
      <h2 align="center" style="font-size: 14px; font-weight: bold; margin: 4px 0 0 0; text-transform: uppercase; text-align: center; font-family: Arial, sans-serif;">SERAH TERIMA PERANGKAT UNTUK PERBAIKAN VENDOR</h2>
    </div>
    ...
    <p class="intro-paragraph" style="margin-bottom: 20px; text-align: justify; line-height: 1.6; font-size: 13px; font-family: Arial, sans-serif;">
      Pada hari ini <strong>${dateFormatted.hari}</strong> Tanggal <strong>${dateFormatted.tglFull}</strong> telah dilakukan penyerahan <strong>${summaryDevicesText}</strong> milik <strong>${cabangText}</strong> kepada pihak Vendor <strong>${vendorDisplayName}</strong> untuk dilakukan perbaikan/pemeliharaan perangkat dengan detail rincian sebagai berikut:
    </p>
    ...
    <table border="1" cellspacing="0" cellpadding="6" width="100%" style="width: 100%; max-width: 480pt; margin: 20px 0 24px 0; border-collapse: collapse; border: 1px solid #000000; font-size: 13px; font-family: Arial, sans-serif;">
      <thead>
        <tr bgcolor="#E0E7FF" style="background-color: #E0E7FF;">
          <th width="40" style="border: 1px solid #000000; padding: 6px; text-align: center; width: 40px; font-weight: bold;">No</th>
          <th width="280" style="border: 1px solid #000000; padding: 6px 10px; text-align: center; width: 280px; font-weight: bold;">Nama / Merek Perangkat</th>
          <th width="160" style="border: 1px solid #000000; padding: 6px; text-align: center; width: 160px; font-weight: bold;">Serial Number (S/N)</th>
        </tr>
      </thead>
      <tbody>
    ...
    <table border="1" cellspacing="0" cellpadding="10" width="100%" style="width: 100%; max-width: 480pt; border-collapse: collapse; border: 1px solid #000000; margin-top: 10px; font-size: 13px; font-family: Arial, sans-serif;">
      <tr>
        <td width="50%" height="150" valign="top" style="width: 50%; border: 1px solid #000000; vertical-align: top; padding: 12px; height: 150px;">
          <div style="font-size: 13px;">Diserahkan oleh (IT Support):</div>
          ...
          <div style="font-size: 11px; color: #555555;">Petugas IT Bank Nagari</div>
        </td>
        <td width="50%" height="150" valign="top" style="width: 50%; border: 1px solid #000000; vertical-align: top; padding: 12px; height: 150px;">
          <div style="font-size: 13px;">Diterima oleh (Vendor):</div>
          <div style="font-size: 11px; color: #444444; margin-bottom: 65px;">Vendor: <strong>${vendorDisplayName}</strong></div>
          <div style="font-size: 13px; font-weight: bold; text-decoration: underline; text-transform: uppercase;">${payload.vendorPic || "........................"}</div>
          <div style="font-size: 11px; color: #555555;">Teknisi / PIC Vendor</div>
        </td>
      </tr>
    </table>
  </div>
...
/** POST /api/reports/berita-acara-vendor — Download Word / Print Berita Acara Vendor */
export async function POST(req: Request) {
  try {
    ...
    const body = (await req.json()) as BeritaAcaraVendorPayload;

    if (!body.vendorName || !body.deviceList || body.deviceList.length === 0) {
      return NextResponse.json({ error: "Nama vendor dan list perangkat wajib diisi." }, { status: 400 });
    }

    const htmlContent = generateBeritaAcaraVendorHtml(body);
    ...
    const filename = `BERITA_ACARA_VENDOR_${body.vendorName.replace(/\s+/g, "_")}.doc`;
    return new Response(htmlContent, {
      headers: {
    ...
    console.error("[POST /api/reports/berita-acara-vendor]", error);
    return NextResponse.json({ error: "Gagal membuat dokumen berita acara vendor." }, { status: 500 });
  }
}

/** GET /api/reports/berita-acara-vendor — Fallback membaca dari database tiket */
export async function GET(req: Request) {
  try {
    ...
    const rawVendor = ticket.wsVendor || "Vendor Perbaikan";
    let vendorName = rawVendor;
    let vendorPic = "";

    const matchVendorPic = rawVendor.match(/^(.*?)\s*\(PIC:\s*(.*?)\)$/i);
    if (matchVendorPic) {
      vendorName = matchVendorPic[1].trim();
      vendorPic = matchVendorPic[2].trim();
    }

    const namaMerekPerangkatStr = resolveNamaPerangkat(ticket.wsMerekKomputer);

    const payload: BeritaAcaraVendorPayload = {
      vendorName,
      vendorPic,
      cabang: ticket.wsCabang || "Utama",
      tgl: ticket.wsTglKeVendor ? new Date(ticket.wsTglKeVendor).toISOString() : new Date().toISOString(),
    ...
    const htmlContent = generateBeritaAcaraVendorHtml(payload);
    ...
    const filename = `BERITA_ACARA_VENDOR_${ticket.noTiket.replace(/[^a-zA-Z0-9-]/g, "_")}.doc`;
    return new Response(htmlContent, {
      headers: {
        "Content-Type": "application/msword; charset=utf-8",
        "Content-Disposition": `attachment; filename="${filename}"`,
    ...
    console.error("[GET /api/reports/berita-acara-vendor]", error);
    return NextResponse.json({ error: "Gagal membuat dokumen berita acara vendor." }, { status: 500 });
  }
}
